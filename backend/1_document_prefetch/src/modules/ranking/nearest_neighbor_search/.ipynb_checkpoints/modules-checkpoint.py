...
class DPBool:
    ...
    def dot(self, a, b ):
        ...
        # self.dp_bool( (1,128,1), (1,256,1), ( a, b, c, a.shape[0], a.shape[1], lookup_table   )  )
        
        batch_size = 1000000
        for pos in range( 0,  a.shape[0], batch_size ):
            self.dp_bool( (1,128,1), (1,256,1), ( a[pos:pos+batch_size], b, c[pos:pos+batch_size], a[pos:pos+batch_size].shape[0], a.shape[1], lookup_table   )  )
...
class DPInt4:
    ...
    def dot(self, a, b ):
        ...
        # self.dp_int4( (1,128,1), (1,256,1), ( a, b, c, a.shape[0], int( a.shape[1]/4 ), lookup_table   )  )
        
        ## this is used since when a.shape[0]>60M there will be OOM error
        batch_size = 1000000
        for pos in range( 0,  a.shape[0], batch_size ):
            self.dp_int4( (1,128,1), (1,256,1), ( a[pos:pos+batch_size], b, c[pos:pos+batch_size], a[pos:pos+batch_size].shape[0], int( a.shape[1]/4 ), lookup_table   )  )
...
class DPInt8:
    ...
    def dot(self, a, b ):
        ...
        # self.dp_int8( (1,128,1), (1,256,1), ( a, b, c, a.shape[0], int( a.shape[1]/4 )  )  )
        
        batch_size = 1000000
        for pos in range( 0,  a.shape[0], batch_size ):
            self.dp_int8( (1,128,1), (1,256,1), ( a[pos:pos+batch_size], b, c[pos:pos+batch_size], a[pos:pos+batch_size].shape[0], int( a.shape[1]/4 )  )  )
...
class BFIndexIPGPU:
    def __init__(self, embeddings, vector_dim, gpu_list = [], internal_precision = "float32", requires_precision_conversion = True, max_copy_ratio = 0.1 ):
        ...
        self.max_copy_ratio = max_copy_ratio
    ...
    def gpu_ranking_kernel( self, query_embedding, embeddings, n, device_id, indices_range= None):
        if indices_range is None:
            ...
            if len(indices_range) > 0:
                ## only alow no more than 10 percent of array copy, this is beneficial for speed and memory usage
                if len(indices_range)/( embeddings.shape[0] + 1e-12 ) < self.max_copy_ratio:
                    distances = self.dp( embeddings[indices_range], query_embedding, self.internal_precision,device_id )
                else:
                    distances = self.dp( embeddings, query_embedding, self.internal_precision,device_id )
                    distances = distances[:,indices_range]
...
class BFIndexIPCPU:
    ...
    def __init__( self, embeddings, vector_dim , num_shards = 1, max_copy_ratio = 0.1 ):
        self.total_num_embeddings = embeddings.shape[0]
    ...
    def cpu_ranking_kernel( self, query_embedding,  n, shard_number, results, indices_range = None ):
        if indices_range is None:
            ...
            if distances.shape[1] <= n:
                I = np.tile( np.arange(distances.shape[1] ), query_embedding.shape[0] ).reshape(  query_embedding.shape[0], distances.shape[1] )
                D = distances
            else:
            ...
            if len(indices_range) > 0:
                if len( indices_range )/( self.embedding_shards[shard_number].shape[0] + 1e-12 ) < self.max_copy_ratio:
...
class BFIndexIP:
    def __init__(self, embeddings, vector_dim, gpu_list = [], internal_precision = "float32", requires_precision_conversion = True ,num_shards = 1, max_copy_ratio = 0.1 ):
        ## num_shards is used for CPU mode
        ## internal_precision and requires_precision_conversion are only used for GPU mode
        if len(gpu_list) == 0:
            self.index = BFIndexIPCPU( embeddings, vector_dim , num_shards, max_copy_ratio = max_copy_ratio )
        else:
            self.index = BFIndexIPGPU( embeddings, vector_dim , gpu_list, internal_precision, requires_precision_conversion, max_copy_ratio = max_copy_ratio  )
    def search( self, query_embedding, n , indices_range = None , requires_precision_conversion = True ):
